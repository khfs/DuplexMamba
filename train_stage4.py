...
class ASR(sb.core.Brain):
    def split_wavs(self, wavs, duration):
        total_size = wavs.shape[1]
        if duration <= 3:
            return torch.split(wavs, [total_size], dim=1)
        else:
            ratios = [3/duration for i in range(int(duration)//3)]
            split_sizes = [int(total_size * ratio) for ratio in ratios]
            remaining_size = total_size - sum(split_sizes)
            
            split_sizes[-1] = split_sizes[-1] + remaining_size

            slices = torch.split(wavs, split_sizes, dim=1)
            return slices
        
        
    def cross_conmamba(self, wavs, wav_lens):
        # compute features
        feats = self.hparams.compute_features(wavs) # (B, T, 80)
        current_epoch = self.hparams.epoch_counter.current
        feats = self.modules.normalize(feats, wav_lens, epoch=current_epoch)
        # forward modules
        # print(feats.shape, wavs.shape)
    ...
    def compute_forward(self, batch, stage):
        ...
        # split wavs
        splited_wavs = self.split_wavs(wavs, batch.duration.item())
        
        enc_out_list = []
        for i in range(len(splited_wavs)):
            enc_out_list.append(self.cross_conmamba(splited_wavs[i], wav_lens))
    ...
    def on_stage_start(self, stage, epoch):
        ...
        if stage == sb.Stage.TRAIN and epoch == 1:
            logger.info("Loading stage3-011_ckpt")
...
def dataio_prepare(hparams):
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "duration", "sig", "wrd", "tokens_eos", "prefix_prompt", "endofspeech", "assistant_prompt", "label1", "final_label"],
    )
