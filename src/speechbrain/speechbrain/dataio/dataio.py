...
def load_data_csv(csv_path, replacements={}):
    ...
    with open(csv_path, newline="") as csvfile:
        ...
        ####### 
        # Regular expression to match "$" followed by an integer or float
        variable_finder = re.compile(r"\$(?!\d+(\.\d+)?)(\w+)")  # Matches $ followed by non-numeric placeholder
        #######
        for row in reader:
            # ID:
            try:
            ...
            for key, value in row.items():
                ...
                #################
                try:
                    # Substitute placeholders only if they don't match "$" followed by a number
                    row[key] = variable_finder.sub(
                        lambda match: str(replacements.get(match[1], match[0])), value
                    )
                #################
                except KeyError:
                    raise KeyError(
