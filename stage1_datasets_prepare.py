...
def prepare_stage1_datasets(
    ...
    >>> prepare_stage1_datasets(data_folder, save_folder, tr_splits, dev_splits, te_splits)
    """
    ...
    check_stage1_datasets_folders(data_folder, splits)
    ...
    for split_index in range(len(splits)):
        ...
        text_lst = get_all_files(
            os.path.join(data_folder, split), match_and=["response.txt"]
        )
...
def check_stage1_datasets_folders(data_folder, splits):
    """
    Check if the data folder actually contains the stage1_datasets.
    ...
    ------
    OSError
        If gpt_omni is not found at the specified path.
    """
    ...
    for split in splits:
        ...
        if not os.path.exists(split_folder):
            err_msg = (
                ...
                "stage1_datasets dataset)" % split_folder
            )
